...
Communs FabMob disponibles (SEULS ces communs peuvent être recommandés) :
{communs_text}

**Étape 2 - Proposition d'expérimentation**

RÈGLE ABSOLUE : Dans "communs_recommandes", tu ne peux citer QUE des communs dont le nom figure EXACTEMENT dans la liste ci-dessus. N'invente aucun nom. Si un commun n'est pas dans la liste, ne le cite pas.

Pour "ressources_open_source_complementaires", tu peux citer des outils open source réels et vérifiables (ex: OpenStreetMap, QGIS, Tracemob). N'invente pas de noms. Si tu n'es pas certain qu'un outil existe, ne le cite pas.

Sois CONCIS : max 20 mots par champ texte, max 3 éléments par liste.
...
{{
  ...
  "communs_recommandes": [
    {{"nom": "Nom EXACT du commun FabMob tel qu'il apparaît dans la liste", "usage": "Usage en 10 mots", "lien": "URL exacte du commun ou chaîne vide si inconnue"}}
  ],
  "ressources_open_source_complementaires": [
    {{"nom": "Nom d'un outil open source réel et vérifiable", "type": "logiciel/données/matériel", "usage": "10 mots max", "lien": "URL officielle ou chaîne vide"}}
  ],
...
def search_transiscope_by_bounds(lat, lng, radius_km=15, limit=25, categories=None):
    """
    Charge tous les acteurs Transiscope puis filtre par distance GPS cote client.
    (boundsJson ignore par l'API - le filtrage geo doit se faire ici)
    """
    from urllib.parse import urlencode
    base_url = "https://transiscope.gogocarto.fr/api/elements.json"
    qs = urlencode({"limit": 2000})
    if categories:
    ...
    try:
        ...
        for el in elements:
            if not isinstance(el, dict):
                continue
            geo = el.get("geo", {})
            el_lat = geo.get("latitude") if isinstance(geo, dict) else None
            el_lng = geo.get("longitude") if isinstance(geo, dict) else None
            if el_lat is None or el_lng is None:
                continue
            try:
                dist = haversine_km(lat, lng, float(el_lat), float(el_lng))
            except Exception:
                continue
            if dist <= radius_km:
                parsed = parse_transiscope_element(el)
                parsed["distance_km"] = round(dist, 1)
                nearby.append((dist, parsed))

        nearby.sort(key=lambda x: x[0])
        results = [r for _, r in nearby[:limit]]
        debug = None if results else f"0 acteur dans {radius_km} km — {len(elements)} elements Transiscope analyses"
...
4. Obtenir le plan complet
    ...
    if st.session_state.step == 1:
        ...
        examples = [
            ...
            "Je veux aider les salariés trop éloignés de leur lieu de travail sans transports en commun",
    ...
    # ── STEP 2 : Décomposition du problème ──────────────────────────────────
        ...
        # Sans key= : variable séparée pour éviter l'erreur Streamlit post-rendu
        if '_decomp_val' not in st.session_state:
            st.session_state._decomp_val = 0  # 0=Oui, 1=Non
        ...
        with col_a:
            validation = st.radio(
                ...
                index=st.session_state._decomp_val
            )
            st.session_state._decomp_val = 0 if validation == "Oui, je valide" else 1
        with col_b:
            if st.button("⬅️ Recommencer", use_container_width=True):
                st.session_state._decomp_val = 0
                st.session_state.step = 1
                st.rerun()

        if validation == "Non, je veux affiner":
            ajout = st.text_area("Précisions supplémentaires :", key="ajout_decomp",
                                  placeholder="Ce qui manque ou est inexact...")
            if st.button("🔄 Relancer l'analyse", type="primary", disabled=not (ajout or "").strip() or not api_key):
                with st.spinner("Réanalyse..."):
                    ...
                    if data:
                        ...
                        st.session_state.decomposition = data
                        st.session_state._decomp_val = 0  # retour à "Oui"
                        st.rerun()
                    else:
    ...
    # ── STEP 3 : Proposition d'expérimentation ───────────────────────────────
        ...
        if '_exp_val' not in st.session_state:
            st.session_state._exp_val = 0  # 0=Oui, 1=Non
        ...
        with col_a:
            validation = st.radio(
                ...
                index=st.session_state._exp_val
            )
            st.session_state._exp_val = 0 if validation == "Oui, je valide" else 1
        with col_b:
            if st.button("⬅️ Retour", use_container_width=True):
                st.session_state._exp_val = 0
                st.session_state.step = 2
                st.rerun()
            ...
            if st.button("🔄 Revoir l'expérimentation", type="primary",
                ...
                with st.spinner("Révision en cours..."):
                    ...
                    if response.startswith("❌"):
                        ...
                        if data:
                            st.session_state.experimentation = data
                            st.session_state._exp_val = 0  # retour à "Oui"
                            st.rerun()
                        else:
                            st.error(f"Erreur de parsing : {err}")
    ...
    # ── STEP 4 : Acteurs locaux ──────────────────────────────────────────────────
        ...
        if st.session_state.transiscope_results is not None:
            ...
            if results:
                # Init selection set
                if 'transiscope_selection' not in st.session_state:
                    st.session_state.transiscope_selection = set()

                nb_sel = len(st.session_state.transiscope_selection)
                st.markdown(f"**{len(results)} acteur(s) trouvé(s) · {nb_sel} sélectionné(s)**")
                st.caption("Cochez les acteurs que vous souhaitez impliquer dans l'expérimentation.")

                for i, r in enumerate(results):
                    dist_str = f" · {r['distance_km']} km" if r.get('distance_km') else ""
                    actor_key = r['nom']
                    is_selected = actor_key in st.session_state.transiscope_selection

                    # Checkbox + expander sur la même ligne
                    col_chk, col_exp = st.columns([1, 12])
                    with col_chk:
                        checked = st.checkbox("", value=is_selected, key=f"chk_t_{i}",
                                              label_visibility="collapsed")
                        if checked and actor_key not in st.session_state.transiscope_selection:
                            st.session_state.transiscope_selection.add(actor_key)
                        elif not checked and actor_key in st.session_state.transiscope_selection:
                            st.session_state.transiscope_selection.discard(actor_key)

                    with col_exp:
                        label = f"{'✅ ' if is_selected else ''}{r['nom']}"
                        if r.get('ville'):
                            label += f" — {r['ville']}"
                        label += dist_str
                        with st.expander(label):
                            ...
                            # Saisie des ressources si sélectionné
                            if is_selected or checked:
                                st.markdown("**💬 Ressources que cet acteur pourrait apporter :**")
                                ressources_key = f"ressources_t_{r['nom']}"
                                if ressources_key not in st.session_state:
                                    st.session_state[ressources_key] = ""
                                st.text_input(
                                    "Ressources (compétences, matériel, réseau...)",
                                    key=ressources_key,
                                    placeholder="Ex: Bénévoles formés, vélos, local de réunion, réseaux sociaux..."
                                )
            else:
        ...
        # ── Résumé des acteurs sélectionnés ─────────────────────────────
        st.markdown("---")
        all_selected = (st.session_state.acteurs_utilisateur or [])
        sel_transiscope = st.session_state.get('transiscope_selection', set())
        nb_transiscope_sel = len(sel_transiscope)
        nb_manuel = len(all_selected)
        total_acteurs = nb_manuel + nb_transiscope_sel
        if total_acteurs > 0:
            parts = []
            if nb_manuel:
                parts.append(f"{nb_manuel} ajouté(s) manuellement")
            if nb_transiscope_sel:
                parts.append(f"{nb_transiscope_sel} sélectionné(s) sur Transiscope")
            st.success(f"✅ **{total_acteurs} acteur(s) prêt(s) à être intégré(s)** — {' · '.join(parts)}")
        else:
            st.info("💡 Vous pouvez passer à l'étape suivante sans acteur, ou en ajouter ci-dessus.")
        ...
        with col_next:
            ...
            if st.button(btn_label, type="primary", use_container_width=True, disabled=not api_key):
                with st.spinner("🤝 Analyse des acteurs et génération du plan..."):
                    # Fusionner acteurs manuels + acteurs Transiscope sélectionnés
                    selection = st.session_state.get('transiscope_selection', set())
                    transiscope_selectionnes = []
                    for r in (st.session_state.transiscope_results or []):
                        if r['nom'] in selection:
                            ressources_key = f"ressources_t_{r['nom']}"
                            ressources_str = st.session_state.get(ressources_key, "")
                            transiscope_selectionnes.append({
                                "nom": r['nom'],
                                "type": "Acteur Transiscope",
                                "ville": r.get('ville', ''),
                                "description": r.get('description', ''),
                                "ressources": [res.strip() for res in ressources_str.split(',') if res.strip()],
                                "contact": r.get('email') or r.get('telephone', ''),
                                "lien": r.get('lien_carte', ''),
                                "categories": r.get('categories', []),
                                "source": "Transiscope",
                            })
                    tous_les_acteurs = (st.session_state.acteurs_utilisateur or []) + transiscope_selectionnes
                    transiscope_for_claude = transiscope_selectionnes  # seulement les sélectionnés
                    ...
                    if not acteurs_data:
                        acteurs_data = {"acteurs_valides": tous_les_acteurs}
