...
def search_transiscope(query, city="", limit=10):
    """Recherche des acteurs de l'économie sociale sur Transiscope."""
    try:
        params = {"limit": limit, "search": f"{query} {city}".strip()}
        resp = requests.get(
            "https://transiscope.gogocarto.fr/api/elements.json",
        ...
        for el in elements[:limit]:
            results.append({
                "nom": el.get("name", el.get("nom", "")),
                "description": el.get("description", el.get("shortDescription", ""))[:200],
                "adresse": el.get("address", {}).get("streetAddress", "") if isinstance(el.get("address"), dict) else el.get("address", ""),
                "ville": el.get("address", {}).get("addressLocality", "") if isinstance(el.get("address"), dict) else "",
                "telephone": el.get("telephone", ""),
                "email": el.get("email", ""),
                "url": el.get("website", el.get("url", "")),
                "lien_carte": f"https://transiscope.gogocarto.fr/map/{el.get('id', '')}",
...
4. Obtenir le plan complet
    ...
    # ── STEP 4 : Acteurs locaux ──────────────────────────────────────────────────
        ...
        st.markdown("""
        Identifiez les associations, entreprises ou collectifs de votre territoire
        qui pourraient s'impliquer dans l'expérimentation.
        L'outil va vérifier leur cohérence et chercher d'autres acteurs sur **Transiscope**
        (carte des alternatives locales).
        """)

        e = st.session_state.experimentation

        # Ville pour la recherche Transiscope
        col_v1, col_v2 = st.columns([2, 1])
        with col_v1:
            city = st.text_input("📍 Ville ou territoire (pour la recherche Transiscope)",
                                  placeholder="Ex: Grenoble, Rennes, Lyon...")
        with col_v2:
            st.markdown("<br>", unsafe_allow_html=True)
            search_query = e.get('nom_experimentation', 'mobilité vélo transport')
        ...
        if 'acteurs_utilisateur' not in st.session_state or st.session_state.acteurs_utilisateur is None:
            st.session_state.acteurs_utilisateur = []

        # Form to add an actor
        with st.expander("➕ Ajouter un acteur", expanded=len(st.session_state.acteurs_utilisateur) == 0):
            col_a1, col_a2 = st.columns(2)
            with col_a1:
                new_nom = st.text_input("Nom de l'acteur", key="new_nom", placeholder="Ex: Association Vélo en Ville")
        ...
        st.markdown("---")
        st.markdown("### 🔍 Recherche sur Transiscope")

        col_s1, col_s2 = st.columns([3, 1])
        with col_s1:
            custom_search = st.text_input("Mots-clés de recherche",
                                           value=search_query,
                                           placeholder="Ex: vélo mobilité covoiturage...")
        with col_s2:
            st.markdown("<br>", unsafe_allow_html=True)
            do_search = st.button("🔎 Rechercher", use_container_width=True)

        if 'transiscope_results' not in st.session_state:
            st.session_state.transiscope_results = None

        if do_search:
            with st.spinner("Recherche sur Transiscope..."):
                results = search_transiscope(custom_search, city, limit=12)
        ...
        if st.session_state.transiscope_results is not None:
            ...
            if results:
                st.markdown(f"**{len(results)} acteur(s) trouvé(s) sur Transiscope :**")
                for r in results:
                    with st.expander(f"🏢 {r['nom']} — {r.get('ville', '')}"):
                        ...
                        st.markdown(f"[Voir sur Transiscope]({r['lien_carte']})")
            else:
                st.info("Aucun résultat trouvé sur Transiscope pour ces mots-clés. Essayez des termes plus généraux (ex: 'vélo', 'mobilité', 'transport').")
