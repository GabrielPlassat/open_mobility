...
def search_transiscope_by_bounds(lat, lng, radius_km=15, limit=20):
    """Recherche Transiscope par zone géographique (boundsJson)."""
    import math
    # 1 degree lat ≈ 111km
    ...
    try:
        resp = requests.get(
            ...
            params={"boundsJson": json.dumps(bounds), "limit": limit},
            timeout=20
        )
        if not resp.ok:
            return [], f"Erreur HTTP {resp.status_code}"
        raw = resp.json()
        # GoGoCarto peut renvoyer plusieurs formats
        if isinstance(raw, list):
            elements = raw
        elif isinstance(raw, dict):
            elements = (raw.get("data", {}) or {}).get("elements", [])
            if not elements:
                elements = raw.get("elements", raw.get("features", []))
        else:
            elements = []
        return [parse_transiscope_element(el) for el in elements if el.get("name") or el.get("nom")], None
    except Exception as e:
        return [], str(e)
    ...
    # ── STEP 4 : Acteurs locaux ──────────────────────────────────────────────────
        ...
        if st.session_state.commune_lat:
            ...
            if do_transiscope:
                with st.spinner(f"Recherche dans un rayon de {radius_km} km autour de {st.session_state.commune_label}..."):
                    ...
                    if err:
                        st.warning(f"Avertissement API Transiscope : {err}")
        ...
        if st.session_state.transiscope_results is not None:
            ...
            if results:
                ...
                st.info(f"Aucun acteur trouvé sur Transiscope dans un rayon de {radius_km} km. "
                        f"Essayez d'augmenter le rayon ou vérifiez que Transiscope couvre bien votre territoire.")
                st.markdown("💡 [Ouvrir Transiscope directement](https://transiscope.gogocarto.fr/) "
                           f"pour vérifier manuellement.")
