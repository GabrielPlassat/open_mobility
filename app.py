...
4. Obtenir le plan complet
    ...
    # ── STEP 2 : Décomposition du problème ──────────────────────────────────
        ...
        # La clé du widget est LA source de vérité (Streamlit gère la persistence seul)
        if "radio_decomp_validation" not in st.session_state:
            st.session_state["radio_decomp_validation"] = "Oui, je valide"
            ...
            if st.button("🔄 Relancer l'analyse", type="primary", disabled=not (ajout or "").strip() or not api_key):
                with st.spinner("Réanalyse..."):
                    ...
                    if data:
                        ...
                        st.session_state.decomposition = data
                        # Remettre le radio à "Oui" via SA propre clé
                        st.session_state["radio_decomp_validation"] = "Oui, je valide"
                        st.rerun()
                    else:
    ...
    # ── STEP 3 : Proposition d'expérimentation ───────────────────────────────
        ...
        if 'radio_exp_validation' not in st.session_state:
            st.session_state['radio_exp_validation'] = "Oui, je valide"
        ...
        with col_a:
            validation = st.radio(
                ...
                # index ignoré quand key est présent - session_state['radio_exp_validation'] fait foi
